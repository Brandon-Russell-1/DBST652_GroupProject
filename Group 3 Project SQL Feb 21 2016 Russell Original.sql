...
/* Create view */
/*This view shows Time on Shelf for items still in stock*/
CREATE OR REPLACE VIEW ItemTOSInStock_View AS
  SELECT i.Item_ID, i.Item_Name AS Name, i.Total_Qty AS Total_Quantity, ROUND(SYSDATE - s.Date_Received) AS ToS_Days, sd.Shipment_Details_ID, sd.Shipment_Qty
    FROM Item i LEFT OUTER JOIN ShipmentDetails sd ON sd.Item_ID = i.Item_ID
      JOIN Shipment s ON s.Shipment_ID = sd.Shipment_ID
        WHERE sd.Shipment_Qty > 0               
          ORDER BY  i.Item_ID, ToS_Days DESC;
              
DESCRIBE ItemTOSInStock_View;

/*This view shows items out of stock*/
CREATE OR REPLACE VIEW ItemZeroQty_View AS
  SELECT i.Item_ID, i.Item_Name AS Name, i.Total_Qty AS Quantity
    FROM Item i LEFT OUTER JOIN ShipmentDetails sd ON sd.Item_ID = i.Item_ID
      JOIN Shipment s ON s.Shipment_ID = sd.Shipment_ID
        WHERE i.Total_Qty <= 0
         ORDER BY  i.Item_ID;
  
DESCRIBE ItemZeroQty_View;
...
SELECT /*fixed*/ * FROM ItemTOSInStock_View;
SELECT /*fixed*/ * FROM ItemZeroQty_View;   
